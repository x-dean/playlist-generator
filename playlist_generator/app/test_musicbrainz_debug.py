#!/usr/bin/env python3
"""
Debug script to test MusicBrainz BPM lookup with detailed logging.
"""

import os
import sys
import logging
from musicbrainzngs import set_useragent

# Add the app directory to the Python path
sys.path.insert(0, os.path.dirname(os.path.abspath(__file__)))

# Setup detailed logging
logging.basicConfig(level=logging.DEBUG, format='%(asctime)s %(levelname)s %(message)s')
logger = logging.getLogger(__name__)

# Set MusicBrainz user agent
set_useragent("playlista-debug", "1.0", "debug@example.com")

from music_analyzer.feature_extractor import AudioAnalyzer

def test_musicbrainz_bpm_debug():
    """Test MusicBrainz BPM lookup with detailed debugging."""
    
    print("Testing MusicBrainz BPM lookup with detailed logging...")
    print("=" * 60)
    
    # Initialize AudioAnalyzer
    analyzer = AudioAnalyzer()
    
    # Test cases with known artists/titles
    test_cases = [
        ("Daft Punk", "Get Lucky"),
        ("The Weeknd", "Blinding Lights"),
        ("Calvin Harris", "This Is What You Came For"),
        ("Avicii", "Wake Me Up"),
        ("David Guetta", "Titanium"),
    ]
    
    for artist, title in test_cases:
        print(f"\n{'='*40}")
        print(f"Testing: {artist} - {title}")
        print(f"{'='*40}")
        
        # Create dummy metadata
        metadata = {
            'artist': artist,
            'title': title
        }
        
        # Test the external BPM lookup
        try:
            bpm = analyzer._get_external_bpm("/dummy/path.mp3", metadata)
            if bpm is not None:
                print(f"✅ Found BPM: {bpm:.1f}")
            else:
                print("❌ No BPM found")
        except Exception as e:
            print(f"❌ Error: {e}")
            import traceback
            print(f"Full traceback: {traceback.format_exc()}")
    
    print(f"\n{'='*60}")
    print("Test completed!")

if __name__ == "__main__":
    test_musicbrainz_bpm_debug() 