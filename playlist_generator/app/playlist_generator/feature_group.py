...
class FeatureGroupPlaylistGenerator:
    ...
    def __init__(self, cache_file: str):
        ...
        logger.debug(f"Initialized FeatureGroupPlaylistGenerator with cache file: {cache_file}")
    ...
    def generate(self, features_list: List[Dict[str, Any]]) -> Dict[str, Dict[str, Any]]:
        ...
        """
        logger.debug(f"Starting feature group playlist generation with {len(features_list)} tracks")
        ...
        try:
            ...
            for f in features_list:
                ...
                if None in (bpm, centroid, danceability):
                    skipped_tracks += 1
                    logger.debug(f"Skipping track with invalid data: {file_path}")
                    continue
                ...
                if key is not None and 0 <= key <= 11:
                    ...
                    logger.debug(f"Track {file_path}: key={key_group}, bpm={bpm}, energy={energy_group}, mood={mood_group}")
            ...
            logger.info(f"Feature group generation complete: {len(playlists)} playlists created from {processed_tracks} tracks (skipped {skipped_tracks})")
            ...
            import traceback
            logger.error(f"Feature group generation error traceback: {traceback.format_exc()}")
            return {} 