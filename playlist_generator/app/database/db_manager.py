...
class DatabaseManager:
    ...
    def _init_db(self):
        ...
        try:
            ...
            import traceback
            logger.error(f"Database init error traceback: {traceback.format_exc()}")
            raise
        ...
        try:
            with sqlite3.connect(self.db_path) as conn:
                ...
                if row:
                    ...
                    logger.debug(f"Successfully retrieved cache entry for key: {key}")
                    return value
                else:
            ...
            import traceback
            logger.error(f"Cache get error traceback: {traceback.format_exc()}")
            return None
    
    def cleanup_cache(self, max_age_hours: int = 24) -> int:
        """Clean up old cache entries."""
        logger.debug(f"Cleaning up cache entries older than {max_age_hours} hours")
        try:
            with sqlite3.connect(self.db_path) as conn:
                cursor = conn.cursor()
                cursor.execute("""
            ...
            import traceback
            logger.error(f"Cache cleanup error traceback: {traceback.format_exc()}")
            return 0
    ...
    def save_tags(self, file_path: str, tags: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            import traceback
            logger.error(f"Save tags error traceback: {traceback.format_exc()}")
            return False
        ...
        try:
            with sqlite3.connect(self.db_path) as conn:
                ...
                if row:
                    ...
                    logger.debug(f"Successfully retrieved tags for file: {file_path}")
                    return tags
                else:
