...
class FileDiscovery:
    ...
    def __init__(self, music_dir: str = '/music', failed_dir: str = '/music/failed_files', 
        ...
        self.valid_extensions = ('.mp3', '.wav', '.flac', '.ogg', '.m4a', '.aac', '.opus')
    ...
    def get_files_for_analysis(self, db_files: Set[str], failed_db_files: Set[str], 
        ...
        if force:
            ...
            files_to_analyze = [f for f in all_files if f not in failed_db_files]
            logger.info(f"Force mode: {len(files_to_analyze)} files to process")
        else:
            # Normal mode: only process new files
            files_to_analyze = [f for f in all_files if f not in db_files and f not in failed_db_files]
            logger.info(f"Normal mode: {len(files_to_analyze)} new files to process")
    ...
    def cleanup_removed_files(self, db_files: Set[str]) -> List[str]:
        ...
        if removed_from_disk:
            logger.info(f"Found {len(removed_from_disk)} files in database that no longer exist on disk")
    ...
    def validate_file_paths(self, filepaths: List[str]) -> List[str]:
        ...
        logger.info(f"Validated {len(valid_files)} out of {len(filepaths)} files")
        return valid_files 