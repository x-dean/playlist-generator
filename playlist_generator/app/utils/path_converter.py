...
class PathConverter:
    ...
    def __init__(self, host_library: str = None, container_music: str = '/music'):
        self.host_library = host_library or os.getenv('HOST_LIBRARY', '/root/music/library')
        self.container_music = container_music
        logger.debug(f"Initialized PathConverter: host_library={self.host_library}, container_music={self.container_music}")
    ...
    def host_to_container(self, host_path: str) -> str:
        ...
        try:
            ...
            if not host_path.startswith(self.host_library):
                logger.warning(f"Host path {host_path} doesn't start with {self.host_library}")
                return host_path
            ...
            logger.debug(f"Converted host path {host_path} -> container path {container_path}")
            return container_path
            
        except Exception as e:
            logger.error(f"Error converting host path {host_path} to container: {str(e)}")
            import traceback
            logger.debug(f"Host to container conversion error traceback: {traceback.format_exc()}")
            return host_path
    ...
    def container_to_host(self, container_path: str) -> str:
        ...
        try:
            ...
            if not container_path.startswith(self.container_music):
                logger.warning(f"Container path {container_path} doesn't start with {self.container_music}")
                return container_path
            
            # Convert container path to host path
            relative_path = os.path.relpath(container_path, self.container_music)
            host_path = os.path.join(self.host_library, relative_path)
            host_path = os.path.normpath(host_path)
            
            logger.debug(f"Converted container path {container_path} -> host path {host_path}")
            return host_path
            
        except Exception as e:
            logger.error(f"Error converting container path {container_path} to host: {str(e)}")
            import traceback
            logger.debug(f"Container to host conversion error traceback: {traceback.format_exc()}")
            return container_path
    ...
    def get_relative_path(self, path: str) -> Optional[str]:
        ...
        try:
            ...
            if normalized_path.startswith(self.host_library):
                relative_path = os.path.relpath(normalized_path, self.host_library)
                logger.debug(f"Got relative path from host library: {relative_path}")
                return relative_path
            
            # Try to get relative path from container music
            if normalized_path.startswith(self.container_music):
                relative_path = os.path.relpath(normalized_path, self.container_music)
                logger.debug(f"Got relative path from container music: {relative_path}")
                return relative_path
            
            logger.warning(f"Path {path} doesn't start with either host library or container music")
            return None
            
        except Exception as e:
            logger.error(f"Error getting relative path from {path}: {str(e)}")
            import traceback
            logger.debug(f"Get relative path error traceback: {traceback.format_exc()}")
            return None
    ...
    def normalize_path(self, path: str, target_type: str = 'auto') -> str:
        ...
        try:
            ...
            import traceback
            logger.debug(f"Normalize path error traceback: {traceback.format_exc()}")
            return path
    ...
    def get_path_info(self, path: str) -> dict:
        ...
        try:
            ...
            import traceback
            logger.debug(f"Get path info error traceback: {traceback.format_exc()}")
            return {} 