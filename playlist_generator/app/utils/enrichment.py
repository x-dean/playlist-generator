...
musicbrainzngs.set_useragent("playlist-generator", "1.0", "https://github.com/your-repo")
...
def enrich_musicbrainz(artist: str, title: str) -> Optional[Dict[str, Any]]:
    ...
    try:
        ...
        result = musicbrainzngs.search_recordings(artist=artist, title=title, limit=1)
        
        if result and 'recording-list' in result and result['recording-list']:
            recording = result['recording-list'][0]
            logger.debug(f"Found MusicBrainz recording: {recording.get('id', 'unknown')}")
            ...
            if 'tag-list' in recording:
                ...
                if genres:
                    metadata['genre'] = ', '.join(genres[:3])  # Limit to top 3 genres
...
def enrich_lastfm(artist: str, title: str) -> Optional[Dict[str, Any]]:
    ...
    try:
        ...
        if response.status_code == 200:
            data = response.json()
            logger.debug(f"Last.fm API returned status {response.status_code} for {artist} - {title}")
            ...
            if 'track' in data:
                ...
                if 'name' in track:
                    ...
                    logger.debug(f"Extracted Last.fm title: {metadata['title']}")
                
                if 'artist' in track and 'name' in track['artist']:
                    metadata['artist'] = track['artist']['name']
                    logger.debug(f"Extracted Last.fm artist: {metadata['artist']}")
                
                # Album info
                if 'album' in track and 'title' in track['album']:
                    metadata['album'] = track['album']['title']
                    logger.debug(f"Extracted Last.fm album: {metadata['album']}")
                ...
                if 'toptags' in track and 'tag' in track['toptags']:
                    ...
                    if tags:
                        ...
                        logger.debug(f"Extracted Last.fm genres: {metadata['genre']}")
                ...
                if 'wiki' in track and 'published' in track['wiki']:
                    ...
                    if published:
                        try:
                            year = published.split(' ')[-1][:4]  # Extract year from date
                            if year.isdigit():
                                metadata['year'] = year
                                logger.debug(f"Extracted Last.fm year: {metadata['year']}")
                ...
                logger.debug(f"No track data found in Last.fm response for {artist} - {title}")
                return None
        else:
        ...
        import traceback
        logger.debug(f"Last.fm request error traceback: {traceback.format_exc()}")
        return None
    except Exception as e:
        logger.warning(f"Last.fm enrichment failed for {artist} - {title}: {e}")
        import traceback
...
def enrich_metadata(artist: str, title: str) -> Dict[str, Any]:
    ...
    if mb_data:
        ...
        logger.debug(f"MusicBrainz enrichment successful, got {len(mb_data)} fields")
    ...
    logger.debug(f"Metadata enrichment complete for {artist} - {title}: {len(enriched_data)} fields")
    return enriched_data

def merge_metadata(existing: Dict[str, Any], new: Dict[str, Any]) -> Dict[str, Any]:
    """Merge existing metadata with new enriched data."""
    logger.debug(f"Merging metadata: existing={len(existing)} fields, new={len(new)} fields")
    ...
    for key, value in new.items():
        if key not in merged or not merged[key]:
            ...
            logger.debug(f"Field {key} already exists, keeping existing value: {merged[key]}")
